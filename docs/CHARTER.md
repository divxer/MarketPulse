...
- **Research-trustworthiness evidence chain (adopted 2026-06-11, after reviewing Vibe-Trading's design docs).** The durable lesson from that project is not its agent/swarm machinery (a complexity tax paid for LLM-autonomous trading — MarketPulse's deterministic design does not owe that tax and must not import it) but its trustworthiness discipline. Three chains, in dependency order, each answering "how do I know I'm not fooling myself":
  1. **策略可信 / strategy trust** — statistical validation on backtest + hit-rate results: permutation (shuffle) controls against false positives in strategy/ticker leaderboards, bootstrap Sharpe confidence intervals, walk-forward consistency. Cheap, deterministic, buildable during the north-star accrual wait. **Recalibrated 2026-06-12 (ROI order, locked):** P1 permutation test (this PR — the `permutation_test` CLI answers "is there edge at all" with a selection-corrected best-of-N champion check) > P2 bootstrap CI > P3 shadow 2a > P4 walk-forward (last: current samples are too thin to split). Identity note: MarketPulse is an **Evidence Engine** ("is this worth believing?"), not a Research Engine ("what else is worth trying?") — statistical validation completes our chain, it does not make us Vibe-Trading. **Future roadmap candidate:** a Research Sandbox with a Promotion Gate (fast experiment layer that may NEVER touch the North Star or the production ledger; promotion into the evidence layer is explicit).
     **Research Sandbox Experiment 1 — swarm_research shadow arm (spec 2026-06-12, this PR; formerly drafted as "Phase 8c-1", renamed because independent of the 8a/8b ML chain):** a NEW verdict source — the NAS-hosted Vibe-Trading swarm (`investment_committee` preset over HTTP) — recorded as ordinary `ai_analysis` events with `payload.strategy="swarm_research"`, graded by the SAME permutation pipeline as the deterministic-prompt arms. Pure research: **isolated from allocator/execution/North-Star** (NOT a strategy-definitions entry — that dir feeds the risk gate; the arm lives only in event payload), default OFF, manual batch CLI. Pre-registered: once ≥30 resolved h5 outcomes accrue, permutation A — `p_system<0.05` ⇒ swarm has edge worth pursuing (8c-2 detectors justified); else archive. Results live in run output, not this fact layer. **Status — technical validation complete ✅ (2026-06-15):** the full chain (provider → Vibe swarm → VERDICT contract → `parse_verdict` → EvaluationEvent → permutation pipeline) was proven end-to-end on a live single-AAPL run (event 263, `VERDICT: neutral`). Key learning: the stock `investment_committee` preset templates no `{goal}`, so a self-report VERDICT requires a **dedicated `swarm_research_investment_committee` preset** that owns the contract at the PM `system_prompt` level; the `trust_env=False` fix (PR #160) defeats the container's ambient HTTP_PROXY. First measured throughput: ≈16.5 min and ≈48 LLM round-trips per ticker (timeout default 1500s). Remaining = ops/config only: persist the preset in the Vibe repo, then one MarketPulse PR (default preset + timeout 1500 + drop `_GOAL_SUFFIX`). See [spec](superpowers/specs/2026-06-12-research-sandbox-1-swarm-research-design.md).
