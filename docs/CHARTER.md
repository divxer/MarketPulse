...
- **Phase 7d–7h reconciliation deepening — STATUS: DORMANT / FUTURE INTEGRATION (reframed 2026-06-11).** Not deprecated and not "no broker account exists." Phase 7a–c are **complete**; the IBKR paper account `DUE411848` is the **intended target account** for future MarketPulse-originated execution, not a dead account. 7d–7h are blocked by a single clear, removable milestone — **"IBKR execution routing not enabled"** — not by paper-trade volume.
  - **Unblock condition:** enable 7b IBKR execution routing (MarketPulse → 7b → IBKR paper → fills). Once orders originate from MarketPulse, the `execDetails` / `commissionReport` Gateway-API callbacks supply orderId / fill / partial-fill / commission / execution-time in real time — a **better primary execution source than Flex** (Flex stays as historical reconciliation backstop). Only then do 7d (simulator calibration) … 7h (active reconciliation) have feedstock.
  - **Why dormant now:** the account has not traded in years; its only holding (AMSC) is a years-old static position. No execution stream flows through IBKR regardless of Flex config, so `broker_execution_snapshot = 0` is correct, not a bug. The 7a Flex parser (`.//Trade` → `broker_execution_snapshot`) is verified complete and will ingest executions the moment the report contains them.
