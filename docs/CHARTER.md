...
- **Phase 8b ML predictions** — gated on the 8a+8b shadow experiment (above) showing statistically meaningful improvement in `ai_verdict_hit_rate_h5`. If no meaningful improvement after sufficient sample accumulation, 8b is abandoned and 8a stays parked/frozen. "Statistically meaningful" and "sufficient sample" are experiment-plan decisions made at unpark.
  - **Pre-registered acceptance criterion (locked 2026-06-12, BEFORE the experiment runs):** the permutation A test is the 8a/8b verdict, not an auxiliary analysis. Post-8a, the v5 arm must show permutation `p_system < 0.05` on its own h5 sample; `p > alpha` means 8a is a more complicated way of producing noise and is abandoned per the existing rule. Locking the criterion pre-run forecloses post-hoc reinterpretation.
  - **Question reframed by the 2026-06-12 first run:** the baseline (v4) shows NO h5 predictive skill (p=0.859, observed below random) — so the 8a design review must first answer "**is the AI verdict worth keeping at all?**", not "how do we make it better?". Working hypothesis to examine: the D-pillar may currently be an **explainer** (news → LLM → narrative verdict) rather than a **predictor**; if so, the right move may be pairing the explainer with a genuine prediction layer rather than enriching the explainer's inputs.
- **IBKR account `DUE411848` + Phase 7d–7h — STATUS: DORMANT / STAGED FUTURE INTEGRATION (reframed 2026-06-11).** Not deprecated, not "no account exists." Phase 7a–c are **complete**, but per their own specs they only ever scoped **paper-account read-sync (7a) + a manual CLI order-write pilot (7b) + reconciliation (7c)** — the 7b Goal is explicit: prove the order API "*without connecting strategy output to broker execution*." So **strategy→IBKR routing was never specced or built**; it is a not-yet-designed future phase, not a committed plan. Evidence (2026-06-11): all 16 `broker_order_intent` rows are `operator_source='cli'`, dated 2026-05-25, `transmit=false` (staged-not-submitted) or connection-timeout; zero strategy-routed, zero filled. `broker_execution_snapshot = 0` is therefore **correct, not a bug** — the `.//Trade` parser is verified complete and will ingest fills the moment a real execution stream exists.

  - **Authoritative-ledger principle (locked 2026-06-11):** research phase → the **internal SQLite Paper Portfolio is the source of truth**, the broker is a shadow. Live phase → the inversion: **IBKR executions become truth** and positions/NAV derive from `broker_execution_snapshot` (real partial fills, commissions, fees, dividends, splits replace internal assumptions). These are not in conflict — they are the same system at different maturity stages. We stay internal-ledger-authoritative **now**; broker-first is a deliberate future inversion, not a current refactor (north-star still accruing, 8a not started, 7d has no data).

  - **`DUE411848` lifecycle:** (1) *Broker Integration Sandbox* — **now** (connection + order-command path proven via CLI, transmit-safe). (2) *Shadow Account* — **system validation, not strategy validation**: paper answers "would the strategy make money"; shadow answers "will it survive contact with the real broker." Split into two sub-stages: **(2a) passive shadow** — read-only, no orders; re-price paper fills against an independent/broker data source and track a **data tracking error** (validates data, calendar, pricing assumptions; zero execution infrastructure, so it may come early without violating evidence-before-infra); **(2b) active shadow** — strategy fans out to **both** internal paper **and** IBKR paper (7b routing + `transmit=true`); broker fills sync back read-only; track an **execution tracking error** (paper_return vs shadow_return) validating routing / fills / commission / slippage / partial-fills / status-sync. North-star stays on the internal ledger throughout. 2b is what first produces `execDetails → broker_execution_snapshot` feedstock and unblocks 7d–7h; **the gate from stage 2 to stage 3 is 2b's execution tracking error staying below a threshold over a sustained window** (threshold/window are spec-time decisions). (3) *Production Paper Trading Account* — strategy genuinely runs through it. (4) *Live-trading pre-validation environment* — gate before real money (still behind the `MP_IBKR_ALLOW_LIVE` + north-star ≥ 5%/90d brake).

  - **Unblock condition for 7d–7h:** reach lifecycle stage 2 (Shadow Execution) — i.e. design + build the missing "strategy → 7b place path" phase **and** flip `transmit` from the safety-staged `false` to `true`. Then `execDetails` / `commissionReport` supply orderId / fill / partial-fill / commission / execution-time in real time — the **primary** execution source; Flex demotes to a historical reconciliation backstop.

  - **Ops decision (2026-06-11):** the daily Flex sync job (`run_flex_sync`) was **disabled** to stop alert noise (5/6 recent runs failed on container DNS resolution of `gdcdyn.interactivebrokers.com`, syncing a dormant account with no business value). Disabled by clearing `IBKR_FLEX_TOKEN` in prod `stack.env` (job self-skips before any network call). **All code kept** — repository, models, snapshot schema, sync pipeline, `ib-gateway` service. Re-enable = restore `IBKR_FLEX_TOKEN` (and mirror in the Portainer stack editor for durability).
- **Research-trustworthiness evidence chain (adopted 2026-06-11, after reviewing Vibe-Trading's design docs).** The durable lesson from that project is not its agent/swarm machinery (a complexity tax paid for LLM-autonomous trading — MarketPulse's deterministic design does not owe that tax and must not import it) but its trustworthiness discipline. Three chains, in dependency order, each answering "how do I know I'm not fooling myself":
  1. **策略可信 / strategy trust** — statistical validation on backtest + hit-rate results: permutation (shuffle) controls against false positives in strategy/ticker leaderboards, bootstrap Sharpe confidence intervals, walk-forward consistency. Cheap, deterministic, buildable during the north-star accrual wait. **Recalibrated 2026-06-12 (ROI order, locked):** P1 permutation test (this PR — the `permutation_test` CLI answers "is there edge at all" with a selection-corrected best-of-N champion check) > P2 bootstrap CI > P3 shadow 2a > P4 walk-forward (last: current samples are too thin to split). Identity note: MarketPulse is an **Evidence Engine** ("is this worth believing?"), not a Research Engine ("what else is worth trying?") — statistical validation completes our chain, it does not make us Vibe-Trading. **Future roadmap candidate:** a Research Sandbox with a Promotion Gate (fast experiment layer that may NEVER touch the North Star or the production ledger; promotion into the evidence layer is explicit).
     **Research Sandbox Experiment 1 — swarm_research shadow arm (spec 2026-06-12, this PR; formerly drafted as "Phase 8c-1", renamed because independent of the 8a/8b ML chain):** a NEW verdict source — the NAS-hosted Vibe-Trading swarm (`investment_committee` preset over HTTP) — recorded as ordinary `ai_analysis` events with `payload.strategy="swarm_research"`, graded by the SAME permutation pipeline as the deterministic-prompt arms. Pure research: **isolated from allocator/execution/North-Star** (NOT a strategy-definitions entry — that dir feeds the risk gate; the arm lives only in event payload), default OFF, manual batch CLI. Pre-registered: once ≥30 resolved h5 outcomes accrue, permutation A — `p_system<0.05` ⇒ swarm has edge worth pursuing (8c-2 detectors justified); else archive. Results live in run output, not this fact layer. **Status — COMPLETE ✅ (2026-06-15):** the full chain (provider → Vibe swarm → VERDICT contract → `parse_verdict` → EvaluationEvent → permutation pipeline) was proven end-to-end on the **production default path** (no env overrides) — event 264, `VERDICT: neutral`, `provenance.preset=swarm_research_investment_committee`, `research_only=True`. The question RS-1 de-risked ("can a swarm verdict be produced and enter the evaluation system?") is answered yes. **Next gate (unchanged):** accumulate ≥30 resolved h5 outcomes, then the pre-registered permutation A — `p_system<0.05` ⇒ pursue (8c-2 detectors justified); else archive the arm. Key learnings now in the fact layer: the stock `investment_committee` preset templates no `{goal}`, so a self-report VERDICT requires the **dedicated `swarm_research_investment_committee` preset** that owns the contract at the PM `system_prompt` level (PR #162); the `trust_env=False` fix (PR #160) defeats the container's ambient HTTP_PROXY; swarm runtime varies a lot (measured 16.5/18.4/27.5 min/ticker, ≈48 LLM round-trips), so the synchronous poll timeout is a stopgap at 2400s (PR #163) and these are split out as **Research Sandbox 1.1 (Swarm verdict-collection hardening)**, a separate experiment to be chartered via its own brainstorm → spec → plan. **RS-1.1 priority (locked 2026-06-15): P0 = Final Price Enforcement** — swarm `event_price` MUST come from an `is_final=True` bar (else abstain); RS-1 validation exposed that the swarm path takes yfinance *provisional* closes (AAPL 291.13 vs 296.33 for the same `as_of`), re-digging the provisional-price hole the `is_final`/`finalize_provisional_bars` governance was built to close. Charter principle: *research outcomes must be anchored to final prices; provisional prices are not eligible for event creation.* P1 async finalizer, P2 provenance accuracy, P3 cost — all rank below P0 (statistical validity > engineering experience). See [RS-1 spec](superpowers/specs/2026-06-12-research-sandbox-1-swarm-research-design.md) and [RS-1.1 backlog](superpowers/specs/2026-06-15-research-sandbox-1.1-backlog.md).
     **First run recorded (2026-06-12, prod, seed=42, N=10,000):** h5 n=110 — observed overall hit rate 24.5% vs permutation null 28.1%, p_system=0.859; best strategy momentum_breakout (n=8) 50.0% vs best-of-N null 35.6%, p_best=0.128; interpretation **`neither`**. h1 n=190 corroborates (p_system=0.713, `neither`). **Standing conclusion until new evidence: no statistical evidence of edge exists; the observed direction is slightly below random.** This is the evidence chain's first complete answer (data: fixed ✓ / execution: roadmapped / strategy: no evidence), and the first time "is there edge?" moved from an opinion question to an evidence question. 110 samples is not a final verdict — but "no supporting evidence" already holds, and the process produced no explanatory escape hatches.
     **Post-first-run resequencing (locked 2026-06-12):** P3 Shadow 2a > P4 Bootstrap CI > P5 8a design review > P6 Walk-Forward. Bootstrap demoted: at n=110 it would only confirm wide intervals; Shadow 2a advances the one chain (execution trust) with no current deliverable.
  2. **数据可信 / data trust** — **final vs provisional bar governance.** `price_cache` pins the first-fetched bar forever (PK ticker+date) with no finality concept. **2026-06-11 audit: 24 provisional rows confirmed in prod, including SPY 2026-06-10 pinned at a midday price (730.72, fetched 12:30 ET) that flowed into the 2026-06-10 AND 2026-06-11 NAV snapshots — 2 of 9 north-star days contaminated.** Required fix: `is_final`/`finalized_at` on `price_cache`; a post-close finalize pass refreshes provisional bars; NAV / north-star / backtest consume `is_final=true` only; the two contaminated snapshots get rebuilt (`is_rebuilt` mechanism already exists). This item feeds the P2 price-freshness data-trust precondition above and is promoted from idea to required work. **Shipped 2026-06-11 (PR1)** as the first piece of the **Data Freshness & Provenance Layer** — `price_cache` is no longer a plain cache but cache + data-quality state; future evolution (upstream corrections, splits/dividend restatements) adds provenance fields like `correction_version`/`source_revision` to this layer rather than new ad-hoc columns. **Future hardening (PR2):** evaluation/backtest read paths consume only-final bars (deferred per the 2026-06-11 review — the NAV leg was the proven production defect; the research-path filter is defense-in-depth once the FinalizeJob exists). Also deferred to PR2+: split fallback telemetry into `fallback_to_older_final` vs `unpriced_due_to_provisional`; a `verify_freshness` CLI scripting the deploy checks.
  3. **执行可信 / execution trust** — the shadow account's tracking error (lifecycle stage 2 above): proof that paper results survive contact with real broker mechanics before any capital does. **2a deliverable (this PR):** `python -m marketpulse.cli.pricing_audit` — the Independent Pricing Audit (spec 2026-06-12; NOT broker shadow): paper fills + the NAV series re-priced against Tencent, pre-registered PASS/FAIL gates; run results live in run output, not in this fact layer, unless later promoted. **Run #1 (2026-06-12): overall = FAIL; findings identified; follow-up required** — investigation lives in `docs/superpowers/specs/2026-06-12-execution-evidence-review-materials.md` (review tier, not facts). **Run #2 (2026-06-12, after the Tencent coverage fix) demonstrated a clean boundary between pre-finality snapshots (systematic drift) and rebuilt snapshots (exact vendor agreement); remediation approved and executed** — the full north-star series (10 days) was rebuilt in one transaction on the healed final-bar chain (`rebuild_reason='post-finality-remediation'`; pre-rebuild values archived in `docs/operations/north-star-baseline-v0.json`). **Run #3: NAV leg PASS at 0.0000% across all 10 days — no single-vendor price illusion in the current canonical chain; fills leg remains FAIL solely on fill #5's frozen revised value, by design (open question: ledger immutability vs revisions).** Governance principle locked: **ledger facts (fills) are immutable; derived metrics (NAV / north-star) are rebuildable on the canonical price chain.**
  - Priority ranking (locked): north-star 90d accrual ≥ shadow account ≥ statistical significance — "能落地" outranks "Sharpe 是否 1.1" for real-money readiness; but build-sequencing differs: chains 1–2 are cheap waiting-period work, 2b shadow waits for an edge signal.
