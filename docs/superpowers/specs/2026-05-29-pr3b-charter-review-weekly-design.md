...
| L19 | Empty `reason` from `paper_audit_event` is normalized to the literal string `"(no reason)"` BEFORE forming `ReasonCount`. This happens in the aggregator's `_top_reasons` helper, not the renderer. (NULL is impossible at the schema level — `reason` column is `nullable=False, default=""` — so in practice only empty-string normalization fires.) Renderer-side `_fmt_reason` operates on already-normalized text. |
...
| L21 | **Index fields are NOT percents.** `portfolio_index`, `spy_index`, `anchor_portfolio_nav` multipliers (e.g. `1.041`) are rendered via `_fmt_index` (`"1.041"`), NEVER `_fmt_pct` (which would produce `"104.1%"` — meaningless). Only `excess_return`, `coverage_ratio`, `tick_success_rate`, `order_rejection_rate` use `_fmt_pct`. Deltas between indices use `_fmt_delta_index` (`"+0.032 vs prior week"`). |
| L22 | **Null observation rule:** every `DiagnosticWeek.value` is `None` when `observations == 0`. For `paper_trade_count` that means: zero snapshot trading days → `value=None`. For `engine_invariant_errors` and `tick_success_rate`: zero `TICK_COMPLETED + ENGINE_INVARIANT_ERROR` events → `value=None`. A week that HAD activity but produced no failures still reports `value=0` truthfully. |
