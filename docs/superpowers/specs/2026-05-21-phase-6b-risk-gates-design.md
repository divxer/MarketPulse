...
| **6b-L16** | **`RiskResult.context` is top-level immutable ONLY — NOT deep-frozen.** Gate authors construct with plain `dict`; `__post_init__` wraps in `MappingProxyType` so external mutation (`result.context["x"] = y`) raises `TypeError`. The guarantee is narrow: **top-level keys cannot be reassigned post-construction**. Nested mutable structures (e.g., `result.context["per_gate"][0]["context"]["foo"] = "bar"`) remain mutable in memory — deep immutability is owned by lock **6b-L17** (`normalize_for_json` materializes a deep, normalized copy at every audit-write boundary, so the persisted audit ledger is the authoritative immutable record). Reviewers must NOT assume `MappingProxyType` is a deep freeze — the spec deliberately splits responsibilities: 6b-L16 = in-memory top-level, 6b-L17 = on-disk deep. |
| **6b-L17** | **Single audit JSON normalizer — owns deep serialization immutability.** All audit-context serialization MUST route through `marketpulse/trading/audit_json.normalize_for_json(obj)`: recursive `Decimal → str`, `datetime`/`date`/`time → isoformat`, `dataclass → dict`, `tuple/list → list`, `Mapping → dict`. The function returns **fresh** containers at every level (no aliasing of input nested structures), which is the project's authoritative point of deep-immutability: the persisted audit ledger is a snapshot independent of any in-memory state that may continue to mutate post-write. `forward_engine._dump` delegates to this util (migrated in T16). New audit-writing code (6f UI, 6g recap, Phase 7 broker) MUST use this util — no per-module normalizers, no inline `json.dumps` of dataclasses. Pairs with lock 6b-L16: 6b-L16 protects in-memory top-level mutation; 6b-L17 owns on-disk deep-copy immutability. |
