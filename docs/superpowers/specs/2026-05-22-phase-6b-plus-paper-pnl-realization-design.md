...
| **6b+L14** | **All `Decimal` prices crossing the price-provider → engine → DB boundary MUST be quantized to 6 decimal places** (`.quantize(Decimal("0.000001"))`) before persisting to `paper_fill.price` / `paper_position.exit_price`. The DB columns are `Numeric(18, 6)`; storing un-quantized Decimals risks subtle round-trip drift (`Decimal("100.123456789")` stored, `Decimal("100.123457")` read → assertions fail in tests). `YFinancePriceProvider.close_on_date` quantizes once at the boundary (`Decimal(str(bar.close)).quantize(Decimal("0.000001"))`); downstream code can trust the value. `StubPriceProvider` callers responsible for passing already-quantized values in `map`. |
| **6b+L15** | **Every committed checkpoint MUST leave `tests/trading/` recoverable.** No intentional broken-state commits, even if labeled "temporary." If a refactor requires both interface change AND call-site updates, they MUST land in the same atomic commit. Specifically: the PriceProvider Protocol rewrite (T3) and the `ForwardExecutionEngine.__init__(price_provider=...)` required kwarg (formerly T7a) MUST commit together. `git bisect` should never land on a "known broken" 6b+ commit. |
| **6b+L16** | **Forward-mode code MUST NOT call `PriceProvider` before exit materialization, AND MUST NOT read `paper_order.horizon_price` for exit P&L.** Enforce by grep test: `tests/trading/test_forward_invariants.py` scans `marketpulse/trading/daily_cycle.py` and `marketpulse/trading/forward_engine.py` (in `_materialize_exit` only) for the strings `price_provider.close_on_date`, `price_provider.horizon_price`, `order.horizon_price`, `paper_order.horizon_price`. Asserts: (a) `daily_cycle.py` has zero matches for any of those (forward path never calls PriceProvider), (b) `_materialize_exit` matches `price_provider.close_on_date` exactly once and `order.horizon_price` zero times. |
...
| 24 | Migration 0011 integration (post-upgrade): seed pre-0011 DB with 5 rows of every 6a event_type, run upgrade, assert (a) all 4 indexes exist with EXACT 0010 names (`ix_paper_audit_ts`, `ix_paper_audit_type_ts`, `ix_paper_audit_order`, `ix_paper_audit_strategy_ts`), (b) all 5 rows preserved (id, timestamp, event_type, order_id, strategy, reason, context all match), (c) CHECK accepts PRICE_UNAVAILABLE inserts | 6b+L6, 6b+L10 |
| 25 | Migration 0011 downgrade integration: with 0 PRICE_UNAVAILABLE rows, downgrade succeeds; CHECK rejects new PRICE_UNAVAILABLE inserts; indexes still named correctly; data preserved | 6b+L10 |
| 26 | Decimal quantization at provider boundary: feed `YFinancePriceProvider` a mock yfinance bar with `close=100.123456789`; assert `ClosePrice.price == Decimal("100.123457")` (HALF_EVEN rounding to 6 decimals) | 6b+L14 |
| 27 | DB round-trip determinism: write a `paper_fill.price = Decimal("100.123457")` via the exit path, read it back, assert byte-equal Decimal (no `100.12346` vs `100.123457` drift) | 6b+L14 |
| 28 | NaN close handling: `fetch_close_on_date` receives a yfinance bar with `Close=NaN` (yfinance returns NaN on some delisted / corporate-action edge days); skip that row and return the next-most-recent valid bar, or None if none. NEVER produce `Decimal("nan")` or raise. | 6b+L5 |
| 29 | Forward-invariant grep (lock 6b+L16): `tests/trading/test_forward_invariants.py` reads `marketpulse/trading/daily_cycle.py` source and asserts `price_provider` / `horizon_price` / `paper_order.horizon_price` appear ZERO times. Reads `_materialize_exit` source in `forward_engine.py` and asserts `price_provider.close_on_date` appears exactly once and `order.horizon_price` zero times. | 6b+L16 |
